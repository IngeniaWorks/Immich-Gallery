    ...
    var body: some View {
        ...
        .onChange(of: focusedTarget) { _, newValue in
            ...
            if let index = viewModel.slides.firstIndex(where: { $0.id == slideID }) {
                viewModel.handleShelfFocusChange(index: index)
            }
        }
...
private extension PauseShelfView {
    ...
    func slideRow(_ item: SlideItemRow) -> some View {
        ...
        let isCurrent = viewModel.currentSlideIndex == item.index
...
private struct SlideItemRow: Identifiable {
    ...
    init(index: Int, slide: SlideItem) {
        id = slide.id
        self.index = index
        self.slide = slide
    }
...
private extension PauseShelfView {
    @ViewBuilder
    func slideThumbnail(for slide: SlideItem, isFocused: Bool, isCurrent: Bool) -> some View {
